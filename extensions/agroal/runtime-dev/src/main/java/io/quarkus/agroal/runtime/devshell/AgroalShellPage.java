package io.quarkus.agroal.runtime.devshell;
...
public class AgroalShellPage extends BaseExtensionPage {
    ...
    private void renderTablesView(Buffer buffer) {
        ...
        buffer.setString(1, row - 1, "Datasources", Style.create().cyan().bold());
        row++;
        ...
        // Right panel: Tables
        row = 3;
        buffer.setString(rightPanelStart - 1, row - 1, "Tables", Style.create().cyan().bold());
        row++;
        ...
        // Footer
        renderFooter(buffer, "[Tab] Switch panel  [Enter] View table data");
    }
    ...
    public boolean handleKey(int key, AppContext ctx) {
        ...
        if (viewMode == ViewMode.SQL_INPUT) {
            return handleSqlInputKey(key, ctx);
        } else if (viewMode == ViewMode.DATA) {
            return handleDataViewKey(key, ctx);
        } else {
            return handleTablesViewKey(key, ctx);
        }
    }

    private boolean handleTablesViewKey(int key, AppContext ctx) {
        // Tab to switch between datasource and table list
        if (key == '\t' || key == KeyCode.TAB) {
            // Toggle focus between lists (simple implementation - just let both handle keys)
            ctx.requestRedraw();
            return true;
        }

        // Let datasource list handle navigation
        ...
        // Let table list handle navigation
    ...
    private boolean handleDataViewKey(int key, AppContext ctx) {
        switch (key) {
            case KeyCode.ESCAPE:
                viewMode = ViewMode.TABLES;
                columns.clear();
    ...
    private boolean handleSqlInputKey(int key, AppContext ctx) {
        switch (key) {
            case KeyCode.ESCAPE:
