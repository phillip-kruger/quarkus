...
public class ExtensionPageFactory {
    ...
    private static boolean matchesPage(String namespace, ShellPageInfo pageInfo) {
        ...
        String pageId = pageInfo.id().toLowerCase();

        // Direct containment check
        if (lowerNamespace.contains(pageId) || pageId.contains(lowerNamespace)) {
            LOG.debugf("    matchesPage: namespace=%s, pageId=%s -> MATCH (containment)", namespace, pageId);
            return true;
        }

        // Strip "quarkus-" prefix and compare (e.g., "quarkus-agroal" -> "agroal")
        String strippedNamespace = lowerNamespace.replace("quarkus-", "").replace("io.quarkus.", "");
        if (strippedNamespace.equals(pageId) || strippedNamespace.contains(pageId) || pageId.contains(strippedNamespace)) {
            LOG.debugf("    matchesPage: namespace=%s, pageId=%s, stripped=%s -> MATCH (stripped)",
                    namespace, pageId, strippedNamespace);
            return true;
        }

        LOG.debugf("    matchesPage: namespace=%s, pageId=%s, stripped=%s -> NO MATCH",
                namespace, pageId, strippedNamespace);
        return false;
    }
