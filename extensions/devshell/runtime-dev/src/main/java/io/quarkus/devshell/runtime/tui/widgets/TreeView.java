...
public class TreeView<T> {
    ...
    public void render(Buffer buffer, int startRow, int startCol) {
        if (flattenedNodes.isEmpty()) {
            buffer.setString(startCol, startRow, "(empty)", Style.create().gray());
            return;
        }
        ...
        for (int i = scrollOffset; i < endIdx; i++) {
            ...
            // Determine style
            if (selected) {
                nameStyle = Style.create().white().onBlue();
            } else if (node.isDirectory) {
                nameStyle = Style.create().cyan().bold();
            } else {
        ...
        if (scrollOffset > 0) {
            buffer.setString(startCol + width - 2, startRow, "\u25B2", indicatorStyle);
        }
        if (endIdx < flattenedNodes.size()) {
            buffer.setString(startCol + width - 2, startRow + visibleRows - 1, "\u25BC", indicatorStyle);
        }
    }
