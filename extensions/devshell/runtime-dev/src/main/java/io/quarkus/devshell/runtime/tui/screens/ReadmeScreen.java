...
public class ReadmeScreen implements Screen {
    ...
    private void findAndLoadReadme(AppContext ctx) {
        loading = true;
        error = null;
        ctx.requestRedraw();
        ...
        try {
            ...
            if (foundPath != null) {
                ...
                loadReadmeContent(ctx, foundPath);
            } else {
                loading = false;
    ...
    private void loadReadmeContent(AppContext ctx, String path) {
        try {
    ...
    public boolean handleKey(int key, AppContext ctx) {
        ...
        int contentHeight = ctx.getHeight() - 5;
