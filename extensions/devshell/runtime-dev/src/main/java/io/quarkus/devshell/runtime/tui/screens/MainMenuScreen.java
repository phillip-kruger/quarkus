...
public class MainMenuScreen implements Screen {
    ...
    public void render(Frame frame, AppContext ctx) {
        ...
        if (selected != null) {
            ...
            if (selected == MenuItem.EXTENSIONS) {
                List<ShellExtension> extensions = ctx.getExtensions();
    ...
    private void openMenuItem(MenuItem item, AppContext ctx) {
        switch (item) {
            case EXTENSIONS:
                List<ShellExtension> extensions = new ArrayList<>(ctx.getExtensions());
                ...
                ctx.navigateTo(new ExtensionsListScreen(extensions));
                break;
            case CONFIGURATION:
                ctx.navigateTo(cdiScreen(ConfigurationScreen.class));
                break;
            case WORKSPACE:
                ctx.navigateTo(new WorkspaceScreen());
                break;
            case ENDPOINTS:
                ctx.navigateTo(cdiScreen(EndpointsScreen.class));
                break;
            case CONTINUOUS_TESTING:
                ctx.navigateTo(cdiScreen(ContinuousTestingScreen.class));
                break;
            case DEV_SERVICES:
                ctx.navigateTo(new DevServicesScreen());
                break;
            case BUILD_METRICS:
                ctx.navigateTo(cdiScreen(BuildInfoScreen.class));
                break;
            case README:
                ctx.navigateTo(new ReadmeScreen());
                break;
            case DEPENDENCIES:
                ctx.navigateTo(new DependenciesScreen());
                break;
        }
    }
