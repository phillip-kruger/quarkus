...
public abstract class BaseExtensionPage implements ExtensionPage {
    ...
    protected void renderPanelContent(Buffer buffer, int startRow, int startCol, int panelWidth, int panelHeight) {
        ...
        if (loading) {
            buffer.setString(startCol, row, "Loading...", Style.create().yellow());
            return;
        }

        if (error != null) {
            buffer.setString(startCol, row, truncate(error, panelWidth - 2), Style.create().red());
            return;
        }

        buffer.setString(startCol, row, "Press Enter to view details", Style.create().gray());
    }
    ...
    protected void renderLoading(Buffer buffer, int row) {
        buffer.setString(1, row, "Loading...", Style.create().yellow());
    }

    protected void renderPanelLoading(Buffer buffer, int row, int col) {
        buffer.setString(col, row, "Loading...", Style.create().yellow());
    }

    protected void renderError(Buffer buffer, int row) {
        if (error != null) {
            buffer.setString(1, row, "Error: " + truncate(error, width - 10), Style.create().red());
        }
    }

    protected void renderPanelError(Buffer buffer, int row, int col, int maxWidth) {
        if (error != null) {
            buffer.setString(col, row, truncate(error, maxWidth - 2), Style.create().red());
        }
    }
    ...
    protected void renderFooter(Buffer buffer, String instructions) {
        ...
        buffer.setString(1, height - 2, footerText, Style.create().gray());
    }
    ...
    /**
     * Creates a JsonObject using the Vert.x classloader as TCCL to avoid
    ...
    private static JsonObject createJsonObject(String json) {
        ...
        try {
            Thread.currentThread().setContextClassLoader(JsonObject.class.getClassLoader());
    ...
    /**
     * Creates a JsonArray using the Vert.x classloader as TCCL to avoid
     * ServiceConfigurationError when QuarkusJacksonFactory is discovered
     * from the application classloader but JsonFactory is from a different classloader.
     */
    private static JsonArray createJsonArray(String json) {
        ClassLoader original = Thread.currentThread().getContextClassLoader();
        try {
            Thread.currentThread().setContextClassLoader(JsonArray.class.getClassLoader());
    ...
    protected int renderTabBar(Buffer buffer, int row) {
        ...
        for (int i = 0; i < tabNames.length; i++) {
            ...
            if (i == currentTabIndex) {
                buffer.setString(col, row, label, Style.create().bold().reversed());
            } else {
                buffer.setString(col, row, label, Style.create().gray());
            }
            col += label.length() + 1;
        }
