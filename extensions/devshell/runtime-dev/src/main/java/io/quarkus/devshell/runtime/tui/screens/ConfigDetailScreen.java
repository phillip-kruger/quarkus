...
public class ConfigDetailScreen implements Screen {
    ...
    public void render(Frame frame, AppContext ctx) {
        ...
        if (config.description != null && !config.description.isEmpty()) {
            ...
            // Wrap description
            List<String> lines = wrapText(config.description, contentWidth);
    ...
    private void renderHeader(Buffer buffer, int width) {
        String title = " Configuration Detail ";
    ...
    public boolean handleKey(int key, AppContext ctx) {
        if (deleteConfirmMode) {
            return handleDeleteConfirmKey(key, ctx);
        }
        if (editMode) {
            return handleEditKey(key, ctx);
        }
        ...
        switch (key) {
            ...
            case 'E':
                if (isEditable()) {
                    ...
                    ctx.setStatus("This property cannot be edited (not a runtime property)");
                }
                return true;
            ...
            case 'D':
                ...
                if (currentValue != null && !currentValue.isEmpty()) {
                    ...
                    ctx.setStatus("Property has no value to delete");
                }
                return true;
    ...
    private boolean handleEditKey(int key, AppContext ctx) {
        switch (key) {
            case KeyCode.ESCAPE:
                // Cancel edit mode
                editMode = false;
    ...
    private boolean handleDeleteConfirmKey(int key, AppContext ctx) {
        switch (key) {
            case 'y':
            case 'Y':
                deleteProperty(ctx);
    ...
    private void deleteProperty(AppContext ctx) {
        saving = true;
        deleteConfirmMode = false;
        saveError = null;
        saveSuccess = null;
    ...
    private void saveValue(AppContext ctx) {
