...
public class ListView<T> {
    ...
    public void render(Buffer buffer, int startRow, int startCol) {
        for (int i = 0; i < visibleRows; i++) {
            ...
            if (itemIndex < items.size()) {
                ...
                String stripped = stripAnsi(label);
                ...
                if (selected) {
                    ...
                    buffer.setString(startCol + 2, row, text, Style.create().reversed());
                } else {
                    String text = AnsiRenderer.fixedWidth(stripped, width - 4);
                    buffer.setString(startCol, row, "  " + text, Style.EMPTY);
                }
            }
        }
    ...
    private void renderScrollIndicator(Buffer buffer, int startRow, int col) {
        ...
        if (scrollOffset > 0) {
            buffer.setString(col, startRow, "\u25B2", Style.create().gray());
        }

        if (scrollOffset < maxOffset) {
            buffer.setString(col, startRow + visibleRows - 1, "\u25BC", Style.create().gray());
        }
    }
