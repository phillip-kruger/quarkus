...
public class ProviderBasedPage extends BaseExtensionPage {
    ...
    private void renderItem(AppContext ctx, ShellPageData.Item item, int maxWidth) {
    ...
    protected void renderPanelContent(Buffer buffer, int startRow, int startCol, int panelWidth, int panelHeight) {
        ...
        for (int i = 0; i < maxItems; i++) {
            ...
            renderPanelItem(buffer, item, panelWidth - 2);
            row++;
        }
    ...
    private void renderPanelItem(Buffer buffer, ShellPageData.Item item, int maxWidth) {
        String label = item.getLabel();
        String value = item.getValue();
