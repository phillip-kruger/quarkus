...
public class ArcShellPage extends BaseExtensionPage {
    ...
    public void render(Frame frame, AppContext ctx) {
        ...
        switch (contentType) {
            case TAB_BEANS -> renderTableView(buffer, ctx, row, beanTable, "CDI Beans", beansCount);
            case TAB_OBSERVERS -> renderTableView(buffer, ctx, row, observerTable, "Observers", observersCount);
            case TAB_INTERCEPTORS -> renderTableView(buffer, ctx, row, interceptorTable, "Interceptors", interceptorsCount);
            case TAB_DECORATORS -> renderTableView(buffer, ctx, row, decoratorTable, "Decorators", decoratorsCount);
            case TAB_EVENTS -> renderEventsView(buffer, ctx, row);
            case TAB_REMOVED -> renderTableView(buffer, ctx, row, removedTable, "Removed Components", removedCount);
        }
    ...
    private <T> void renderTableView(Buffer buffer, AppContext ctx, int startRow, TableView<T> table, String title,
            int count) {
    ...
    private void renderEventsView(Buffer buffer, AppContext ctx, int startRow) {
    ...
    public boolean handleKey(int key, AppContext ctx) {
        ...
        if (key == KeyCode.TAB || (key >= '1' && key <= '9')) {
            boolean handled = super.handleKey(key, ctx);
            if (handled) {
