...
public class HibernateOrmShellPage extends BaseExtensionPage {
    ...
    public HibernateOrmShellPage() {
        ...
        this.entityTable = new TableView<EntityInfo>()
                ...
                .addColumn("Class", e -> e.className, 40)
                .addColumn("Table", e -> e.tableName, 25);

        // Named Queries table
        this.queryTable = new TableView<QueryInfo>()
                .addColumn("Name", q -> q.name, 30)
                .addColumn("Type", q -> q.type, 10)
                .addColumn("Query", q -> truncate(q.query, 50), 50);
    ...
    public boolean handleKey(int key, AppContext ctx) {
        ...
        if (viewMode == ViewMode.HQL_RESULTS) {
            return handleHQLResultsKey(key, ctx);
        }

        if (hqlInputMode) {
            return handleHQLInputKey(key, ctx);
        }
    ...
    private boolean handleHQLInputKey(int key, AppContext ctx) {
        switch (key) {
            case KeyCode.ESCAPE:
                hqlInputMode = false;
                redraw();
    ...
    private boolean handleHQLResultsKey(int key, AppContext ctx) {
        switch (key) {
            case KeyCode.ESCAPE:
    ...
    private static class PersistenceUnitInfo {
        final String name;
        final int entityCount;
        final int queryCount;
        final boolean reactive;

        PersistenceUnitInfo(String name, int entityCount, int queryCount, boolean reactive) {
    ...
    private static class EntityInfo {
        ...
        EntityInfo(String name, String className, String tableName, String persistenceUnit) {
    ...
    private static class QueryInfo {
        ...
        QueryInfo(String name, String query, String type, boolean cacheable, String lockMode, String persistenceUnit) {
            this.name = name != null ? name : "unknown";
            this.query = query != null ? query : "";
            this.type = type != null ? type : "JPQL";
