        ...
        try {
            ...
            if (array != null) {
                for (int i = 0; i < array.size(); i++) {
                    ...
                    if (encoded.contains("\"Retry\"")) {
                        annotations.add("Retry");
                        retryCount++;
                    }
                    if (encoded.contains("\"CircuitBreaker\"")) {
                        annotations.add("CB");
                        circuitBreakerCount++;
                    }
                    if (encoded.contains("\"Timeout\"")) {
                        annotations.add("Timeout");
                        timeoutCount++;
                    }
                    if (encoded.contains("\"Fallback\"")) {
                        annotations.add("Fallback");
                        fallbackCount++;
                    }
                    if (encoded.contains("\"Bulkhead\"")) {
                        annotations.add("Bulkhead");
                        bulkheadCount++;
                    }
                    if (encoded.contains("\"RateLimit\"")) {
