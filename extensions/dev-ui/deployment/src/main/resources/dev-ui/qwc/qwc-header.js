...
export class QwcHeader extends observeState(LitElement) {
    ...
    render() {
        ...
        <div class="top-bar">
            ...
            <div class="right-bar">
                ${this._rightSideNav}
                <span class="dayNightIcon" @click="${this._toggleTheme}">
                    <vaadin-icon icon="font-awesome-solid:${themeState.theme.icon}"></vaadin-icon>
                </span>
            </div>
        </div>
        `;
    ...
    _toggleTheme(){
        this._changeTheme(themeState.theme.name === "dark");
    }
    ...
    _changeTheme(isDark){
        ...
        themeState.theme = themeState.changeTo(name);
    }
